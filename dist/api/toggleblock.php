    ...
    if(isset($_GET['id'])) {
        ...
        if($stat->rowCount() > 0) {
            ...
            if($data['friend1_id'] == $id) {
                ...
                if($data['friend1_block'] == 0) {
                    $stat = $con->prepare("UPDATE friending SET friend1_block = ? WHERE (friend1_id = ?) AND (friend2_id = ?)");
                ...
                if($data['friend2_block'] == 0) {
                    $stat = $con->prepare("UPDATE friending SET friend2_block = ? WHERE (friend2_id = ?) AND (friend1_id = ?)");
                    $stat->execute(array('1', $id, $_GET['id']));
                } else {
                    $stat = $con->prepare("UPDATE friending SET friend2_block = ? WHERE (friend2_id = ?) AND (friend1_id = ?)");
                    $stat->execute(array('0', $id, $_GET['id']));
